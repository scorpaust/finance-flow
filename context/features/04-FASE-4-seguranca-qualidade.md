# FASE 4 — Segurança, Qualidade e Preparação para Produção

> Pré-requisito: Fases 1, 2 e 3 concluídas. Esta fase não adiciona
> funcionalidades novas — endurece o que já existe antes da Fase 5
> (publicação).
...
- [ ] Introduzir validação de schema (Zod) em todos os endpoints
      ...
      categories, groups, subscription)
...
- [ ] Rever CORS para o domínio de produção (bloquear origens não
      autorizadas)
- [ ] Segredos (Mongo URI, Stripe keys, RevenueCat keys, session secret)
      apenas via variáveis de ambiente — nunca no repositório

### 3. Webhooks
- [ ] Verificação de assinatura obrigatória em todos os webhooks (Stripe,
      RevenueCat) — rejeitar pedidos não assinados
...
- [ ] Testes de integração para endpoints críticos: auth, transactions CRUD,
      subscription checkout/webhook (com mocks do Stripe/RevenueCat)
...
- [ ] Testar app em dispositivo Android de gama baixa (ou emulador com
      recursos limitados) para validar fluidez das animações da Fase 3 e do
      modelo de ML
