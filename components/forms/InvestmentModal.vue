    ...
    <div class="modal-overlay" @click.self="$emit('close')">
      <div class="modal-content max-w-lg w-full" @click.stop>
        ...
        <div class="flex items-center justify-between mb-5">
          <div>
            ...
            <p class="text-white/40 text-xs mt-0.5">
              {{ isEditing ? 'Atualiza os dados desta posição' : 'Regista uma posição do teu portfolio' }}
            </p>
          </div>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <!-- Nome -->
          <div>
            <label class="form-label" for="inv-name">Investimento *</label>
            <input
              id="inv-name"
              v-model="form.name"
              type="text"
              class="form-input"
              maxlength="80"
              placeholder="Ex: FTSE All-World ETF, Certificados de Aforro..."
              required
              autofocus
            />
          </div>
          ...
          <div>
            <label class="form-label" for="inv-class">
              Classe de ativo <span class="text-white/30 font-normal">(opcional)</span>
            </label>
            <select id="inv-class" v-model="form.assetClass" class="form-select">
              <option value="">— Sem classe —</option>
              <option v-for="c in ASSET_CLASSES" :key="c" :value="c">{{ ASSET_CLASS_LABEL[c] }}</option>
            </select>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="form-label" for="inv-initial">Inicial (€) *</label>
            ...
            <div>
              <label class="form-label" for="inv-date">Data *</label>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="form-label" for="inv-reinf">Reforço (€)</label>
              <div class="input-group">
                <span class="input-prefix font-medium">€</span>
                <input
                  id="inv-reinf"
            ...
            <div>
              <label class="form-label" for="inv-value">Situação (€) *</label>
              <div class="input-group">
                <span class="input-prefix font-medium">€</span>
                <input
                  id="inv-value"
          ...
          <p class="text-white/30 text-xs -mt-2">
            Reforço é o total já acrescentado ao valor inicial. Situação é o valor atual da posição.
          </p>
          ...
          >
            <div class="text-xs text-white/50">
              <p>Investido: <span class="text-white/80 font-medium">{{ formatCurrency(preview.invested) }}</span></p>
              <p v-if="preview.valid" class="mt-0.5">
                {{ preview.gain >= 0 ? 'Ganho' : 'Perda' }}:
          ...
          <div class="flex gap-3 pt-1">
            ...
            >
              ...
              {{ saving ? 'A guardar...' : isEditing ? 'Atualizar' : 'Guardar' }}
            </button>
          </div>
        </form>
      </div>
...
async function handleSubmit() {
  ...
  if (!name) { formError.value = 'O nome é obrigatório.'; return }
  if (!(initialAmount > 0)) { formError.value = 'Indica um valor inicial maior que zero.'; return }
  if (!Number.isFinite(reinforcement) || reinforcement < 0) { formError.value = 'O reforço não pode ser negativo.'; return }
  if (!Number.isFinite(currentValue) || currentValue < 0) { formError.value = 'Indica a situação atual (valor ≥ 0).'; return }
  if (!form.initialDate) { formError.value = 'A data é obrigatória.'; return }
